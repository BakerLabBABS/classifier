...
### No tip labels
plot(tr, label.offset = 0.03, cex = 0.7, show.tip.label = FALSE)
...
probs_df_percent <- round(probs_df, digits = 2)
## Then use this dataframe to nodelabels after plotting the tree command
nodelabels(node=nodenums, pie=probs_df_percent, piecol=colors, cex=0.5)

## circular plot, if needed
plot.phylo(tr, type = "fan", label.offset = 0.03, cex = 0.7, show.tip.label = FALSE)
plot.phylo(tr, type = "unrooted", label.offset = 0.03, cex = 0.5, show.tip.label = FALSE, use.edge.length = FALSE, node.depth = 2, edge.width = 0.2)
